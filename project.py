...
tab1, tab2, tab3, tab4, tab5 = st.tabs(['Danh sách','Biểu đồ','Phân Nhóm','Phân loại','Dự đoán điểm'])
with tab1:
...
with tab5:
  ...
  kr_model = load_model('model.keras')

  st.title('Dự đoán việc học tiếp')
  col1, col2 = st.columns(2)
  with col1:
      x1 = st.number_input('S-AVG',0.0,10.0, step=0.1)
  with col2:
      x2 = st.number_input('GPA',0.0,10.0, step=0.1)
  button = st.button('Predict', use_container_width=True)
  if button:
      if x2 == 0 or x1 == 0:
          st.snow()
          st.error('Nhập S-AVG, GPA')
      else:
          X = np.array([[x1,x2]])
          y = kr_model.predict(X)
          y_new_label = ['No' if p[0] < 0.5 else 'Yes' for p in y]
          st.balloons()
          st.success(f'Dự đoán việc học tiếp: {y_new_label[0]}')
